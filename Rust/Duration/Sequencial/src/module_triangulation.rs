...
impl TTriangulation {
    ...
    pub fn construct_triangulation_xyz(
        ...
        let start_construct_nodes_time = Instant::now();


        triangulation.construct_nodes(&node_x, &node_y, &node_z);


        let construct_nodes_duration = start_construct_nodes_time.elapsed();
        let start_construct_triangles_time = Instant::now();


        triangulation.construct_triangles();


        let construct_triangle_duration = start_construct_triangles_time.elapsed();
        let start_construct_directed_edges_time = Instant::now();


        triangulation.construct_directed_edges();


        let construct_directed_edge_duration = start_construct_directed_edges_time.elapsed();
        let total_duration = start_total_time.elapsed();

        triangulation.export_to_txt(
            filename,
            total_duration,
            construct_nodes_duration,
            construct_triangle_duration,
            construct_directed_edge_duration
        )?;
        Ok(())
    }
    ...
    pub fn export_to_txt(
        ...
        construct_nodes_duration: Duration,
        construct_triangle_duration: Duration,
        construct_directed_edge_duration: Duration
    ) -> Result<()> {
        // let data_path = "../../../Data/Output/Rust/Duration/Sequencial/";
        let data_path = "./Data/Output/Rust/Duration/Sequencial/";
        ...
        let formated_construct_nodes_duration = format!("{:?}", construct_nodes_duration);
        let formated_construct_triangle_duration = format!("{:?}", construct_triangle_duration);
        let formated_construct_directed_edge_duration = format!("{:?}", construct_directed_edge_duration);

        let mut file = File::create(input_path)?;

        writeln!(file, "TTriangulation:")?;
        writeln!(file, "  instance_id: {}", self.instance_id)?;
        writeln!(file, "  total_duration: {}", formated_total_duration)?;
        writeln!(file, "  construct_nodes_duration: {}", formated_construct_nodes_duration)?;
        writeln!(file, "  construct_triangle_duration: {}", formated_construct_triangle_duration)?;
        writeln!(file, "  construct_directed_edge_duration: {}", formated_construct_directed_edge_duration)?;
