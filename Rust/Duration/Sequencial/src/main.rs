...
fn parse_ltri_entry(s: &str) -> io::Result<(usize, usize, usize)> {
    ...
    let v = v_str.parse::<usize>().map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "valor v inválido"))?;
...
fn main() {
    ...
    let input_filename: Result<String, _> = if args.len() > 2 {args[2].to_string().parse()} else {Ok("1_000.txt".to_string())};
    ...
    let mut lend: Vec<usize> = Vec::new();
    let mut lnew: usize = 0;
    let mut near: Vec<usize> = Vec::new();
    let mut next_tri: Vec<usize> = Vec::new();
    let mut ltri: Vec<Vec<usize>> = Vec::new();
    let mut bnodes: Vec<usize> = Vec::new();
